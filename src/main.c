...
static const lua_Integer MAX_DIMENSION = 32767;
...
static lua_Integer check_dimension(lua_State *L, int index) {
  ...
  luaL_argcheck(L, dimension > 0 && dimension <= MAX_DIMENSION, index,
                "width/height must be in range 1-32767");
  return dimension;
}
 ...
 * mouse coordinates and modifier keys. Returns true if the window is still open
 * and false if it's closed (only on windows right now).
 * @param L Lua state
 * @return Number of return values on the Lua stack
 */
...
static int window_clear(lua_State *L) {
  ...
  // (color uses a narrowing cast to int, but we made sure it's in range)
