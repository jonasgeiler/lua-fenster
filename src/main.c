...
static int lua_fenster_open(lua_State *L) {
  ...
  p_lua_fenster->keys_size =
      sizeof(p_fenster->keys) / sizeof(p_fenster->keys[0]);
...
static int lua_fenster_close(lua_State *L) {
  lua_fenster *p_lua_fenster =
      (lua_fenster *) luaL_checkudata(L, 1, "lua_fenster");
...
static int lua_fenster_loop(lua_State *L) {
  ...
  if (lua_gettop(L) >= 2) {
    ...
    const int64_t delta_frame_time =
        fenster_time() - p_lua_fenster->last_frame_time;
...
static int lua_fenster_set(lua_State *L) {
  ...
  if (x < 0 || x >= p_lua_fenster->p_fenster->width ||
      y < 0 || y >= p_lua_fenster->p_fenster->height) {
